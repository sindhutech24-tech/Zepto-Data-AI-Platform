...
This project focuses on the common Titanic survival problem and explores whether factors such as:
...
influence the probability of survival. The trained pipeline demonstrates a standard supervised learning workflow using preprocessing, model comparison, and evaluation metrics.

## Notes

- Scripts use relative paths, so they should be run from inside the `analytics/` directory.
- The trained model is saved in `models/` for later reuse or deployment.
- The project includes both classification and regression tasks, making it a useful reference for end-to-end data science experimentation.

## Summary

This analytics module is a compact example of a production-style data science workflow: data ingestion, cleaning, EDA, feature engineering, modeling, and artifact generation.
